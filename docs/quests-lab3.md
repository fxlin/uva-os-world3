...
**AFTERTHOUGHT** (3/8/25, based on post lab3 survey): 
source-level debugging support is important, esp for quest01 when MMU is not on yet. hopefully the added debugging method (below) solve that problem. (secondary: debugging support for user app) 
...
1. In `boot.S` before the MMU is enabled, QEMU/GDB cannot perform source-level debugging. This means that setting a breakpoint with `b _start` will not work. Explanation: When you instruct GDB to add a breakpoint using a symbol name, GDB looks for the link address of that symbol (e.g., `ffff000000080000`) in the ELF file (debug info) and sets a breakpoint at that address. However, this is not what you want. You need to set a breakpoint at `0x80000`. Therefore, you should use `b *0x80000` in GDB instead.

    - but then if you do single step (`si`) and see those instructions, how do you map them back to the source lines? You need to open the disassembly file (kernel8.asm) and find the corresponding instructions.

    - if you want to inspect the contents of kernel pgtables, you can use GDB to dump memory contents. The symbol is `pg_dir` defined in the linker script, but you need to use PA for GDB, which is the link address subtracted by VA_START.

1. 
Updated: Mar/5/2025. now an elf is automatically generated as below. 
to use it, cf `launch_gdb_boot()` in dbg-rpi3emu.sh. 

Mar/4/2025. Here is a way to enable GDB source-level debugging for code *before* the MMU is enabled:
    - In the linker script (e.g., `kernel/linker-rpi3qemu.ld`), *temporarily* change the start address to 0x80000 (i.e., the PA where the kernel will be loaded to).
    - Build the entire kernel and copy the resultant `kernel8.elf` to a temporary location, e.g., `/tmp/kernel8.elf`.
    - Revert the change in the linker script to its original state.
    - Clean and rebuild the entire kernel. Launch it using GDB as usual.
    - Upon entering GDB, load the debugging symbols from `/tmp/kernel8.elf` using the command `file /tmp/kernel8.elf`.
    - Then try setting breakpoints using symbol names, e.g., `b _start` or `b create_kern_pgtables`; then `c` to continue. 
    GDB should stop at the breakpoint and show the source code.
    - (The above steps can be automated by having a separate linker script and an additional step in Makefile. 
    Basically, you are running a "normal" kernel and load symbols from a "debug" kernel.)

1. If you try to single step (`si` in GDB) from the start, 
you will see the PC starting from 0x0. 
0x0 is where the firmware is located, not our kernel. 
Our kernel8.img is copied to 0x80000 in PA. 
Upon power on, the CPU starts from 0x0, executes through the firmware, and jumps to 0x80000. 
Note: the firmware implementation differs between QEMU and rpi3 hardware, but the behavior described here is the same.
	

1. From prior labs we know GDB can stop at certain kernel symbol. 
Example: to stop at el0_sync, launch the kernel with GDB then do "b el0_sync", and "r". 

1. GDB can also set breakpoint for user code (or kuser code). It just needs the correct virtual address. 
