...
# allow source-level debugging the boot process, before MMU is on. 
# cf linker-rpi3qemu-bootdebug.ld
launch_gdb_boot() {
    # see above
    gnome-terminal --window-with-profile=gdb --geometry 80x80 -- \
    gdb-multiarch -ex "file ./kernel/build-rpi3qemu/kernel8-bootdebug.elf"
}
